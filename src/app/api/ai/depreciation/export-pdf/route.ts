...
async function POST_INNER(req: NextRequest, ctx: { userId: string; orgId: string | null }) {
  try {
    ...
    const { claimId, rcv, age, lifespan, depreciationType, acv, depreciation } = body;

    if (!claimId || rcv === undefined) {
      return NextResponse.json({ error: "claimId and rcv are required" }, { status: 400 });
    }
