/**
 * PHASE 16.6 — AUTO-GENERATED ADJUSTER PACKET
...
✓ Complete weather verification with NOAA/NWS documentation
✓ Detailed damage assessment with photographic evidence
✓ Line-item estimate with current market pricing
✓ Overhead & Profit justification (3+ trades required)
✓ Code upgrade requirements per local building codes
...
async function generateDamageDocumentation(claim: any): Promise<string> {
  ...
  doc += `Observed Damage:\n`;
  doc += `• Shingle damage with granule loss\n`;
  doc += `• Mat damage and bruising\n`;
  doc += `• Ridge cap displacement\n`;
  doc += `• Flashing compromise\n`;
  doc += `• Gutter/downspout damage\n`;
  doc += `• Fascia/soffit impact\n\n`;
...
• Coordinate scheduling between multiple specialized trades
• Ensure proper sequencing of work (tear-off → decking → roofing → gutters → paint)
• Manage material ordering and delivery logistics
• Provide quality control across all phases
• Ensure code compliance for all trades
...
• Ice & Water Shield - Required on all eaves and penetrations
• Drip Edge - Required on all eaves and rakes
• Proper Ventilation - Must meet current ventilation standards
• Attachment Method - Must use current fastening requirements
• Underlayment - Must meet current underlayment specifications
